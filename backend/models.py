...
class ExtractedField(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    claim_id = Column(Integer, ForeignKey("claims.id"), nullable=False)
    field_category = Column(String(100), nullable=False)  # Patient, Policy, Hospital, Clinical, Financial, Documents
    field_name = Column(String(100), nullable=False)
    field_value = Column(Text, nullable=True)
    confidence = Column(Float, nullable=True)
    is_manually_corrected = Column(Integer, default=0)  # SQLite doesn't have native bool
...
class HospitalProfile(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    hospital_name = Column(String(255), unique=True, index=True, nullable=False)
    total_claims = Column(Integer, default=0)
