...
def rule_ayush_hospital_not_registered(claim_context: dict) -> Optional[RuleResult]:
    ...
    if db:
        ...
        profile = db.query(HospitalProfile).filter(
            HospitalProfile.hospital_name == fields.get(("hospital", "name"), "")
        ).first()
...
def rule_ayush_bill_amount_outlier(claim_context: dict) -> Optional[RuleResult]:
    ...
    bill_str = fields.get(("financial", "bill_amount")) or fields.get(("financial", "total_bill_amount"))

    if not bill_str:
        return None

    try:
        amount = float(bill_str.replace(",", "").replace("₹", "").replace("Rs", "").strip())
...
def rule_unregistered_ayush_practitioner(claim_context: dict) -> Optional[RuleResult]:
    ...
    qualifications = fields.get(("hospital", "doctor_qualifications"), "").lower()
    doctor = fields.get(("hospital", "treating_doctor")) or fields.get(("clinical", "treating_doctor"))

    if not doctor:
        return None

    has_ayush_qualification = any(q in qualifications for q in AYUSH_QUALIFICATIONS)
