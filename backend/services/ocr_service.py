...
async def run_ocr(file_path: str, mime_type: str | None) -> str:
    ...
    if mime_type and "pdf" in mime_type:
        ...
        if raw.strip() and not raw.startswith("[OCR ERROR]"):
            return raw
        logger.info("PDF had no embedded text — converting to images for vision OCR.")
    ...
    for i, img in enumerate(images):
        logger.info(f"Sending page {i+1}/{len(images)} to Kimi K2.5 on Bedrock for vision OCR.")
