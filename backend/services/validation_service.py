...
REQUIRED_FIELDS = {
    ...
    ("clinical", "icd_code"):       "ICD-10 Diagnosis Code",   # IRDAI mandates coded diagnosis
    # Hospital
...
async def validate_claim(db: Session, claim_id: int) -> dict:
    ...
    fields    = db.query(ExtractedField).filter(ExtractedField.claim_id == claim_id).all()
    ...
    if adm_val and dis_val:
        ...
        if adm_date and dis_date:
            if adm_date > dis_date:
                errors.append("[IRDAI] Admission date is after discharge date — invalid stay period")
            else:
                los = (dis_date - adm_date).days
                if los == 0:
                    warnings.append("Length of stay is 0 days — verify if day-care procedure")
                elif los > 90:
                    warnings.append(f"Unusually long hospitalization: {los} days — requires review")
    ...
    if icd_code:
        ...
        if not fmt_ok:
            errors.append(f"[IRDAI] Invalid ICD-10 code format: {icd_code} — {fmt_msg}")
            code_validation["icd10"] = {"code": icd_code, "valid": False, "message": fmt_msg}
        else:
            # Check plausibility
            if not is_plausible_medical_code(icd_code):
                warnings.append(f"ICD-10 code '{icd_code}' prefix is unusual for a hospitalization claim")
            ...
            if nlm_found:
                ...
                warnings.append(f"ICD-10 code '{icd_code}' not found in NLM registry — may be incorrect")
                code_validation["icd10"] = {
                    "code": icd_code,
                    "valid": False,
        ...
        code_validation["icd10"] = {"code": None, "valid": False, "message": "No ICD-10 code extracted"}
    ...
    if pcs_code:
        ...
        code_validation["pcs"] = {"code": pcs_code, "valid": pcs_ok, "message": pcs_msg}
        if not pcs_ok:
            warnings.append(f"[IRDAI] PCS procedure code format invalid: {pcs_msg}")
    else:
        code_validation["pcs"] = {"code": None, "valid": None, "message": "PCS code not present (required for surgical claims)"}
        warnings.append("No PCS procedure code found — required for surgical / procedural claims")
