...
from services.extraction_service import AIServiceClient
...
async def generate_document_summary(
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.error(f"Summary API Error: {resp.status_code} - {resp.text}")
            return _build_fallback_summary(extracted_fields)
...
def _build_fallback_summary(extracted_fields: dict) -> dict:
    ...
    if name:
        ...
        overview_parts.append("The patient's name could not be determined from the uploaded documents.")

    if weight:
        overview_parts.append(f"The patient's weight was recorded as {weight}.")
    ...
    if department:
        overview_parts.append(f"The patient was seen in the {department} department.")

    adm = financial.get("admission_date", "")
    dis = financial.get("discharge_date", "")
    if adm and dis:
        overview_parts.append(f"The patient was admitted on {adm} and discharged on {dis}.")
    elif adm:
        overview_parts.append(f"The visit/admission date was recorded as {adm}.")
    ...
    if symptoms:
        clinical_parts.append(f"The patient presented with the following symptoms: {symptoms}.")
    if procedure:
    ...
    if vitals:
        clinical_parts.append(f"Vital signs recorded include: {', '.join(vitals)}.")

    if icd:
        clinical_parts.append(f"The medical classification code(s) noted: {icd}.")
    ...
    if meds:
        med_list = ", ".join(meds)
        clinical_parts.append(f"The following medications were prescribed: {med_list}.")
    ...
    if clinical_parts:
        ...
        paragraphs.append("No specific clinical details such as diagnosis, procedures, or medications could be extracted from the documents.")
