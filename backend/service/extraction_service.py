...
async def extract_fields(raw_text: str) -> dict:
    ...
    def fallback_extract(text: str) -> dict:
        ...
        m = re.search(
            r"(?:Patient\s*Name|Name\s*of\s*Patient|Patient)\s*[:\-]\s*([A-Za-z.\s]+?)(?:\s+Age|\s*$|\s*,)", text, re.IGNORECASE)
        if not m:
            # Fallback: capture any capitalized name sequence right before "Age:"
            # Handles garbled OCR labels like "DMme:", "Nme:", etc.
            m = re.search(
                r"(?:^|[:\-]\s*|me:\s*)([A-Z][A-Za-z.\s]{3,50}?)\s+Age\s*[:\-]", text, re.MULTILINE)
        add("patient", "name", m.group(1) if m else None)

        m = re.search(
            r"Age\s*[:\-]\s*(\d+)\s*(?:years?|yrs?|Y)?", text, re.IGNORECASE)
        add("patient", "age", m.group(1) if m else None)

        m = re.search(
            r"(?:Sex|Gender)\s*[:\-]\s*(Male|Female|M|F|Other|Transgender)", text, re.IGNORECASE)
        add("patient", "gender", m.group(1) if m else None)

        m = re.search(
            r"(?:Weight|Wt|WT)\s*[:\-]\s*([\d.]+\s*(?:kg|lbs?)?)", text, re.IGNORECASE)
        add("patient", "weight", m.group(1) if m else None)

        m = re.search(
            r"(?:Height|Ht|HT)\s*[:\-]\s*([\d.]+\s*(?:cm|ft|m|inches?)?)", text, re.IGNORECASE)
        add("patient", "height", m.group(1) if m else None)

        m = re.search(
            r"(?:Blood\s*Group|Blood\s*Type)\s*[:\-]\s*([ABO]{1,2}[+-]?)", text, re.IGNORECASE)
        add("patient", "blood_group", m.group(1) if m else None)

        m = re.search(
            r"(?:Phone|Mobile|Contact|Tel)\s*[:\-]\s*([\d\s+()-]{7,15})", text, re.IGNORECASE)
        add("patient", "phone", m.group(1) if m else None)

        m = re.search(
            r"(?:Patient\s*ID|MRN|MR\s*No|Reg\.?\s*No|UHID)\s*[:\-]\s*([\w\d/-]+)", text, re.IGNORECASE)
        add("patient", "patient_id", m.group(1) if m else None)

        m = re.search(
            r"(?:Father|Father'?s?\s*Name|S/O|D/O|W/O|C/O)\s*[:\-]\s*([A-Za-z.\s]+?)(?:\s*$|\s*,|\n)", text, re.IGNORECASE)
        add("patient", "guardian_name", m.group(1) if m else None)

        m = re.search(
            r"(?:Address|Addr|Residence)\s*[:\-]\s*(.+?)(?:\n|\r|$)", text, re.IGNORECASE)
        ...
        m = re.search(
            r"(?:Hospital(?:\s*Name)?|Nursing\s*Home|Medical\s*Centre|Medical\s*Center|Clinic)\s*[:\-]\s*([A-Za-z\w\s.&]+)", text, re.IGNORECASE)
        if m:
        ...
        m = re.search(
            r"(?:ROHINI|NABH|Registration)\s*(?:ID|No\.?|Number)\s*[:\-]\s*([\w\d/-]+)", text, re.IGNORECASE)
        add("hospital", "registration_id", m.group(1) if m else None)

        m = re.search(
            r"(?:Bed\s*No|Bed|Ward|Room)\s*[:\-]\s*([\w\d\s/-]+?)(?:\s*$|\s*,|\n)", text, re.IGNORECASE)
        add("hospital", "bed_ward", m.group(1) if m else None)

        # ── Doctor / Consultant ──
        m = re.search(
            r"(?:Dr\.?\s*|Doctor\s*[:\-]\s*|Consultant\s*[:\-]\s*|Treated\s*by\s*[:\-]?\s*|Attending\s*Physician\s*[:\-]\s*)([A-Za-z.\s]+?)(?:\s*$|\s*,|\n|(?=\s*(?:MBBS|MD|MS|FRCS|MCh|DNB)))", text, re.IGNORECASE)
        add("hospital", "treating_doctor", m.group(1) if m else None)

        m = re.search(
            r"(?:Speciality|Specialty|Department|Dept)\s*[:\-]\s*([A-Za-z\s/&]+?)(?:\s*$|\s*,|\n)", text, re.IGNORECASE)
        add("hospital", "department", m.group(1) if m else None)

        # ── Clinical Data ──
        m = re.search(
            r"(?:Diagnosis|Primary\s*Diagnosis|Final\s*Diagnosis|Condition|Disease)\s*[:\-]\s*(.+?)(?:\n|\r|$)", text, re.IGNORECASE)
        add("clinical", "diagnosis", m.group(1)[:300] if m else None)

        m = re.search(
            r"(?:ICD|ICD[\s-]*10|ICD[\s-]*Code)\s*[:\-]\s*([A-Z]\d{2,3}(?:\.\d{1,2})?(?:\s*,\s*[A-Z]\d{2,3}(?:\.\d{1,2})?)*)", text, re.IGNORECASE)
        add("clinical", "icd_codes", m.group(1) if m else None)

        m = re.search(
            r"(?:Procedure|Surgery|Operation|Treatment)\s*[:\-]\s*(.+?)(?:\n|\r|$)", text, re.IGNORECASE)
        add("clinical", "procedure", m.group(1)[:300] if m else None)

        m = re.search(
            r"(?:Symptoms?|Chief\s*Complaint|Presenting\s*Complaint|C/O)\s*[:\-]\s*(.+?)(?:\n|\r|$)", text, re.IGNORECASE)
        add("clinical", "symptoms", m.group(1)[:300] if m else None)

        m = re.search(
            r"(?:Medications?|Medicine|Drugs?|Rx)\s*[:\-]\s*(.+?)(?:\n|\r|$)", text, re.IGNORECASE)
        add("clinical", "medications", m.group(1)[:300] if m else None)

        m = re.search(
            r"(?:BP|Blood\s*Pressure)\s*[:\-]\s*(\d{2,3}\s*/\s*\d{2,3})", text, re.IGNORECASE)
        add("clinical", "blood_pressure", m.group(1) if m else None)

        m = re.search(
            r"(?:Pulse|Heart\s*Rate|HR)\s*[:\-]\s*(\d{2,3})\s*(?:bpm|/min)?", text, re.IGNORECASE)
        add("clinical", "pulse", m.group(1) if m else None)

        m = re.search(
            r"(?:Temp|Temperature)\s*[:\-]\s*([\d.]+)\s*(?:°?[FC])?", text, re.IGNORECASE)
        add("clinical", "temperature", m.group(1) if m else None)

        m = re.search(
            r"(?:SpO2|Oxygen\s*Saturation|O2\s*Sat)\s*[:\-]\s*(\d{2,3})\s*%?", text, re.IGNORECASE)
        add("clinical", "spo2", m.group(1) if m else None)

        m = re.search(
            r"(?:Allergy|Allergies|Known\s*Allergies)\s*[:\-]\s*(.+?)(?:\n|\r|$)", text, re.IGNORECASE)
        add("clinical", "allergies", m.group(1)[:200] if m else None)

        # ── Financial Data ──
        m = re.search(
            r"(?:Admission\s*Date|Date\s*of\s*Admission|DOA|Admitted\s*On)\s*[:\-]\s*([\d/.\-]+)", text, re.IGNORECASE)
        add("financial", "admission_date", m.group(1) if m else None)

        m = re.search(
            r"(?:Discharge\s*Date|Date\s*of\s*Discharge|DOD|Discharged\s*On)\s*[:\-]\s*([\d/.\-]+)", text, re.IGNORECASE)
        add("financial", "discharge_date", m.group(1) if m else None)

        m = re.search(
            r"(?:Total\s*(?:Bill|Amount|Charges?)|Grand\s*Total|Net\s*Amount|Bill\s*Amount)\s*[:\-]?\s*(?:Rs\.?|INR|₹)?\s*([\d,]+(?:\.\d{2})?)", text, re.IGNORECASE)
        add("financial", "total_bill_amount", m.group(1) if m else None)

        m = re.search(
            r"(?:Room\s*(?:Rent|Charges?)|Bed\s*Charges?)\s*[:\-]?\s*(?:Rs\.?|INR|₹)?\s*([\d,]+(?:\.\d{2})?)", text, re.IGNORECASE)
        add("financial", "room_charges", m.group(1) if m else None)

        m = re.search(
            r"(?:Medicine\s*Charges?|Pharmacy|Drug\s*Charges?)\s*[:\-]?\s*(?:Rs\.?|INR|₹)?\s*([\d,]+(?:\.\d{2})?)", text, re.IGNORECASE)
        add("financial", "medicine_charges", m.group(1) if m else None)

        m = re.search(
            r"(?:Investigation|Lab\s*Charges?|Diagnostic|Test\s*Charges?)\s*[:\-]?\s*(?:Rs\.?|INR|₹)?\s*([\d,]+(?:\.\d{2})?)", text, re.IGNORECASE)
        add("financial", "investigation_charges", m.group(1) if m else None)

        m = re.search(
            r"(?:Consultation|Doctor'?s?\s*(?:Fee|Charges?)|Professional\s*Fees?)\s*[:\-]?\s*(?:Rs\.?|INR|₹)?\s*([\d,]+(?:\.\d{2})?)", text, re.IGNORECASE)
        add("financial", "consultation_charges", m.group(1) if m else None)

        m = re.search(
            r"(?:OT\s*Charges?|Operation\s*Theatre|Surgical?\s*Charges?)\s*[:\-]?\s*(?:Rs\.?|INR|₹)?\s*([\d,]+(?:\.\d{2})?)", text, re.IGNORECASE)
        add("financial", "ot_charges", m.group(1) if m else None)

        m = re.search(
            r"(?:Discount|Concession)\s*[:\-]?\s*(?:Rs\.?|INR|₹)?\s*([\d,]+(?:\.\d{2})?)", text, re.IGNORECASE)
        add("financial", "discount", m.group(1) if m else None)

        m = re.search(
            r"(?:Paid|Amount\s*Paid|Advance|Deposit)\s*[:\-]?\s*(?:Rs\.?|INR|₹)?\s*([\d,]+(?:\.\d{2})?)", text, re.IGNORECASE)
        add("financial", "amount_paid", m.group(1) if m else None)

        m = re.search(
            r"(?:Balance|Due|Payable|Outstanding)\s*[:\-]?\s*(?:Rs\.?|INR|₹)?\s*([\d,]+(?:\.\d{2})?)", text, re.IGNORECASE)
        ...
        if "financial" not in fields or "admission_date" not in fields.get("financial", {}):
            m = re.search(
                r"(?:Date)\s*[:\-]\s*([\d/.\-]+)", text, re.IGNORECASE)
            add("financial", "admission_date", m.group(1) if m else None)

        # ── Policy / Insurance ──
        m = re.search(
            r"(?:Policy\s*(?:No\.?|Number)|Policy#)\s*[:\-]\s*([\w\d/-]+)", text, re.IGNORECASE)
        add("policy", "policy_number", m.group(1) if m else None)

        m = re.search(
            r"(?:Insurer|Insurance\s*Company?|Insurance\s*Provider)\s*[:\-]\s*([A-Za-z\s&.]+?)(?:\s*$|\s*,|\n)", text, re.IGNORECASE)
        add("policy", "insurer_name", m.group(1) if m else None)

        m = re.search(
            r"(?:TPA|Third\s*Party)\s*[:\-]\s*([A-Za-z\s&.]+?)(?:\s*$|\s*,|\n)", text, re.IGNORECASE)
        add("policy", "tpa_name", m.group(1) if m else None)

        m = re.search(
            r"(?:Claim\s*(?:No\.?|Number|ID)|Claim#)\s*[:\-]\s*([\w\d/-]+)", text, re.IGNORECASE)
        add("policy", "claim_number", m.group(1) if m else None)

        m = re.search(
            r"(?:Member\s*ID|Membership\s*No|Card\s*No)\s*[:\-]\s*([\w\d/-]+)", text, re.IGNORECASE)
        ...
        if "hospital" not in fields or "address" not in fields.get("hospital", {}):
            addr_m = re.search(
                r"(\d+[,.]\s*[A-Za-z][A-Za-z\s,./]+(?:Road|Lane|Street|Nagar|Colony|Block|Sector|District|City|Town|Dhaka|Delhi|Mumbai|Kolkata|Chennai)[A-Za-z\s,./0-9-]*)", text, re.IGNORECASE)
            add("hospital", "address", addr_m.group(
                1)[:200].strip() if addr_m else None)

        # ── Doctor names from "Dr." prefix ──
        if "hospital" not in fields or "treating_doctor" not in fields.get("hospital", {}):
            dr_m = re.search(
                r"((?:Dr|Prof)\.?\s+[A-Za-z.\s]+?)(?:\s*$|\s*,|\n|(?=\s*(?:MBBS|MD|MS|FRCS|FCPS|BDS|DO|DNB)))", text, re.MULTILINE)
            add("hospital", "treating_doctor",
                dr_m.group(1).strip() if dr_m else None)

        # ── Designation / Specialty ──
        spec_m = re.search(
            r"((?:MBBS|MD|MS|FRCS|FCPS|BDS|DO|DNB|MCh|DM|Fellowship)[A-Za-z\s,.()/]*)", text, re.IGNORECASE)
        if spec_m:
            add("hospital", "doctor_qualifications",
                spec_m.group(1).strip()[:200])

        # ── Professional memberships ──
        member_m = re.search(
            r"(Member\s+of\s+[A-Za-z\s().]+)", text, re.IGNORECASE)
        if member_m:
            add("hospital", "professional_membership", member_m.group(1).strip())

        total_fields = sum(len(v) for v in fields.values())
        logger.info(
            f"Regex fallback extracted {total_fields} fields across {len(fields)} categories.")
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.error(
                f"Extraction API Error: {resp.status_code} - {resp.text}")
        ...
        logger.warning(
            "Failing over to regex extraction after repair failure.")
        return fallback_extract(raw_text)
