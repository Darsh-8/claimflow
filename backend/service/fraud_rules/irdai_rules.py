...
def rule_missing_mandatory_documents(claim_context: dict) -> Optional[RuleResult]:
    ...
    for d in docs:
        dt = d.doc_type.value if hasattr(
            d.doc_type, "value") else str(d.doc_type)
...
def rule_pre_authorization_violation(claim_context: dict) -> Optional[RuleResult]:
    ...
    has_preauth = any(
        (d.doc_type.value if hasattr(d.doc_type, "value")
         else str(d.doc_type)) == "pre_auth"
        for d in docs
    )
...
def rule_waiting_period_violation(claim_context: dict) -> Optional[RuleResult]:
    ...
    try:
        ...
        if days_since < 730:
            matched_disease = [
                d for d in WAITING_PERIOD_DISEASES if d in diagnosis]
            return RuleResult(
                rule_id="IRDAI_003",
                severity="HIGH",
                score=30,
...
def rule_cashless_reimbursement_gap(claim_context: dict) -> Optional[RuleResult]:
    ...
    """
    fields = claim_context.get("fields", {})
    final_str = fields.get(("financial", "bill_amount")) or fields.get(
    ...
    try:
        final_amt = float(final_str.replace(",", "").replace(
            "₹", "").replace("Rs", "").strip())
        preauth_amt = float(preauth_str.replace(
