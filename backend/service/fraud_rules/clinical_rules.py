...
def rule_investigation_inconsistency(claim_context: dict) -> Optional[RuleResult]:
    ...
    is_surgery = any(x in procedure for x in [
                     "surgery", "ectomy", "plasty", "otomy"])

    if is_surgery:
        from model.models import DocumentType
        has_lab = any(
            d.doc_type == DocumentType.LAB_REPORT or d.doc_type == "lab_report" for d in docs)
...
def rule_medication_mismatch(claim_context: dict) -> Optional[RuleResult]:
    ...
    is_cancer_diagnosis = any(x in diagnosis for x in [
...
def rule_pre_auth_vs_final_bill_drift(claim_context: dict) -> Optional[RuleResult]:
    ...
    try:
        final_amt = float(final_amount_str.replace(
            ",", "").replace("₹", "").replace("Rs", "").strip())
        preauth_amt = float(preauth_amount_str.replace(
            ",", "").replace("₹", "").replace("Rs", "").strip())
